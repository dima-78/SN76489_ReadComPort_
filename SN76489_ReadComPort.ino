...
void SendByteToSoundChip(byte b)
{
  ...
  // chip's 8 bit data bus.
    ...
    if (PIN_D3 != 255) {  
    digitalWrite(PIN_D7, (b&1)?HIGH:LOW);
    digitalWrite(PIN_D6, (b&2)?HIGH:LOW);
    digitalWrite(PIN_D5, (b&4)?HIGH:LOW);
    digitalWrite(PIN_D4, (b&8)?HIGH:LOW);
    digitalWrite(PIN_D3, (b&16)?HIGH:LOW);
    digitalWrite(PIN_D2, (b&32)?HIGH:LOW);
    digitalWrite(PIN_D1, (b&64)?HIGH:LOW);
    digitalWrite(PIN_D0, (b&128)?HIGH:LOW);
  } else {
    digitalWrite(PIN_D1, LOW);  // prepare shift register for data
    shiftOut(PIN_D0, PIN_D2, MSBFIRST, b); // send data
    digitalWrite(PIN_D1, HIGH); // update display
  }
